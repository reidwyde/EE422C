...
This is a sample script (perform_grading.sh) for assignment 1.  You have to run this script successfully with your program and our testing files.  The script runs two JUNIT testcases on your code using the same script that we will use to grade  your code.
...
Once you have finished your coding, you can place "your_code.java" in the "submissions" directory. Please do not include whitespace in your file names. A Java file has been placed there as an example.

On the ECE servers, you must do the command 'module load maven' before executing the grading script.

Once everything is ready, you can run the "perform_grading.sh" script on your command line. The shells Bash and zsh have been tested. If you have any questions regarding running a shell script, Google it! It's very easy! For example, ./perform_grading.sh usually works. The JUNIT tests are under grading_project.
...
When the script has run successfully, you can find the brief results in the "brief_results.csv" file. You can open it with Microsoft Excel, Numbers, LibreOffice Calc, or any text editor. 
...
If the grading script works once, but does not work for further attempts or if it has some buggy behaviour, please use a fresh copy of all these files, always removing the provided submission file. (Don't worry, the TA version of this script does not have such problems.)
